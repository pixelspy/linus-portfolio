...
function uncode_admin_get_oembed()
{
	...
	if (isset($raw_provider['host']))
	{
		...
		$key .= 'H6ybd_H-r_';
		$key .= 'G3J9xEEUMoplQ';
		$key = 'key' . '=' . apply_filters( 'uncode_google_api_key', $key );
		...
		if ($bare_host[0] === 'youtube') {
			...
			$code = '<img src="' . $json->items[0]->snippet->thumbnails->standard->url . '" />';
			// $code = '<img src="https://i.ytimg.com/vi/' . $idvideo . '/hqdefault.jpg" />';
