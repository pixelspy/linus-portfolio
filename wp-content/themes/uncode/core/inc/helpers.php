...
function uncode_resize_image($media_id, $url, $path, $originalWidth, $originalHeight, $single_width, $single_height = null, $crop = false, $fixed_width = null, $async = false)
{
	// Cast to int
	...
	if (!empty($actual_file_path))
	{
		...
		if ($originalWidth > $targetWidth || $originalHeight > $targetHeight)
		{
			...
			if ( ! $has_meta_data && ot_get_option('_uncode_adaptive_register_meta') === 'on' ) {
 ...
 * @since Uncode 2.3.0
 */
function uncode_adaptive_secondary_featured_image( $post_id, $image_orig_w, $image_orig_h, $single_width, $single_height, $crop, $single_fixed, $is_tax_block ) {
	...
	if ( $adaptive_images === 'on' && $adaptive_images_async === 'on' ) {
		$resized_image['data_async'] = 'data-uniqueid="'.$thumb_id.'-'.uncode_big_rand().'" data-guid="'.(is_array($media_attributes->guid) ? $media_attributes->guid['url'] : $media_attributes->guid).'" data-path="'.$media_attributes->path.'" data-width="'.$image_orig_w.'" data-height="'.$image_orig_h.'" data-singlew="'.$single_width.'" data-singleh="'.$single_height.'" data-crop="'.$crop.'" data-fixed="'.$single_fixed.'"';
	}
