...
function uncode_equeue() {
	...
	if (class_exists( 'WooCommerce' )) {
		...
		wp_enqueue_script( 'woocommerce-uncode', get_template_directory_uri() . '/library/js/' . $folder . 'woocommerce-uncode' . $suffix . '.js', array() , $resources_version, 'all');
...
function uncode_body_classes($classes){
	...
	if ( ot_get_option( '_uncode_woocommerce_atc_notify' ) == 'minicart' ) {
