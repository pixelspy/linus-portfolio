...
if ( class_exists( 'WooCommerce' ) && $dynamic === 'add-to-cart' && is_a( $product_object, 'WC_Product' ) ) {
	...
	echo '<span class="' . esc_attr(trim(implode(' ', $wrapper_class))) . '" '.implode(' ', $div_data_attributes).'><a href="' . $a_href . '" class="custom-link ' . esc_attr(trim($css_class)) . '"' . $title . $target . $onclick . $rel . $lightbox_data . $width . $el_id . '>' . $bigtext_start . do_shortcode($content) . $bigtext_end . '</a></span>';
}
