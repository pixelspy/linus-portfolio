...
if ( ! function_exists( 'ot_type_custom_post_type_select_2' ) ) {
  ...
  function ot_type_custom_post_type_select_2( $args = array() ) {
  	$args['none'] = true;
  	ot_type_custom_post_type_select( $args );
  }
  ...
  function ot_type_custom_post_type_select( $args = array() ) {
	...
	echo '<div class="format-setting type-custom-post-type-select select-with-switching-button ' . ( $has_desc ? 'has-desc' : 'no-desc' ) . '">';
	  ...
	  echo '<div class="format-setting-inner">';
		...
		/* has posts */
		if ( is_array( $my_posts ) && ! empty( $my_posts ) ) {
			if ( !isset( $field_choices ) || $field_choices === '' || !is_array( $field_choices ) || empty( $field_choices ) ) {
				echo '<option value="">' . esc_html__( 'Select...', 'uncode-core' ) . '</option>';
				if ( isset($args['none']) && $args['none']==true ) {
					echo '<option value="none">' . esc_html__( 'None', 'uncode-core' ) . '</option>';
				}
			} else {
