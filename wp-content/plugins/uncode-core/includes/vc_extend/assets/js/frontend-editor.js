...
window.vc || (window.vc = {}),
	...
	function($) {
		...
		}), $(window).off("resize.fixElContainment", function() {
	...
	function($) {
		...
		vc.map = {}, vc.setFrameSize = function(size) {
			...
			vc.message_timeout && ($(".vc_message").remove(), window.clearTimeout(vc.message_timeout)), timeout = timeout || 5500;
	...
	function($) {
		...
		window.vc.events.on("shortcodeView:updated", function(modelId) {
			...
			addControls: function() {
				var parentShortcodeTag, editAccess, parentAllAccess, parentEditAccess, data, parentName, shortcodeTag = this.model.get("shortcode"),
					compiledTemplate = $(this.controls_selector).html(),
					moveAccess = vc.shortcodes.get(this.model.get("parent_id"));
				moveAccess && (parentName = vc.getMapped(moveAccess.get("shortcode")).name, parentShortcodeTag = moveAccess.get("shortcode")), data = vc_user_access().shortcodeAll(shortcodeTag), editAccess = vc_user_access().shortcodeEdit(shortcodeTag), parentAllAccess = vc_user_access().shortcodeAll(parentShortcodeTag), parentEditAccess = vc_user_access().shortcodeEdit(parentShortcodeTag), moveAccess = vc_user_access().partAccess("dragndrop"), data = {
					name: vc.getMapped(shortcodeTag).name,
					tag: shortcodeTag,
			...
			multi_edit: function(e) {
				var children, models = [];
				e && e.preventDefault && e.preventDefault(), this.model.get("parent_id") && (children = vc.shortcodes.get(this.model.get("parent_id"))), children ? (models.push(children), children = vc.shortcodes.where({
		...
		window.InlineShortcodeView_vc_column = window.InlineShortcodeViewContainerWithParent.extend({
			...
			customCssClassReplace: function() {
				var css_classes = this.$el.find(".wpb_column").attr("class"),
					class_match = /.*(vc_custom_\d+).*/,
					class_match = !(!css_classes || !css_classes.match) && css_classes.match(class_match);
	...
	function($) {
		...
		window.InlineShortcodeView_vc_tta_tabs = window.InlineShortcodeView_vc_tta_accordion.extend({
			...
			sectionUpdated: function(model, justAppend) {
				...
				$tabEl.length ? ($element = '<span class="vc_tta-title-text">' + title + "</span>", $element = this.addIcon(model, $element), $tabEl.html($element)) : ($element = '<span class="vc_tta-title-text">' + title + "</span>", $element = this.addIcon(model, $element), $element = $('<li class="vc_tta-tab" data-vc-target-model-id="' + sectionId + '" data-vc-tab><a href="javascript:;" data-vc-use-cache="false" data-vc-tabs data-vc-target="[data-model-id=' + sectionId + ']" data-vc-container=".vc_tta">' + $element + "</a></li>"), !0 !== justAppend && -1 < (index = _.pluck(_.sortBy(vc.shortcodes.where({
					parent_id: this.model.get("id")
				}), function(childModel) {
	...
	function() {
		...
		window.InlineShortcodeView_vc_tab = window.InlineShortcodeViewContainerWithParent.extend({
			...
			clone: function(builder) {
				builder && builder.preventDefault && builder.preventDefault(), builder && builder.stopPropagation && builder.stopPropagation(), vc.clone_index /= 10, this.model.clone().get("params"), builder = new vc.ShortcodesBuilder;
